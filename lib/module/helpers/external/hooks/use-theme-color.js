...
let webColorProbe;
let webColorCanvasContext;
function getWebColorProbe() {
...
function getWebColorCanvasContext() {
  const browser = globalThis;
  const document = browser.document;
  ...
  if (!webColorCanvasContext) {
    ...
    canvas.width = 1;
    canvas.height = 1;
    webColorCanvasContext = canvas.getContext('2d', {
      willReadFrequently: true
    }) ?? undefined;
  }
  return webColorCanvasContext;
}
function serializeBrowserColorValue(color) {
...
function resolveBrowserColorValue(color) {
  ...
  return typeof computedColor === 'string' && computedColor.length > 0 ? serializeBrowserColorValue(computedColor) : color;
}
